               ...
               int dividendDegree, int divisorDegree, int &quotientDegree, int &remainderDegree )
{
   ...
   int j = remainderDegree;
   while (quotientDegree>=0) {
       if (remainder[j] != 0) {
           int buffer[arraySize] = {}, bufferDegree = remainderDegree;
           quotient[quotientDegree] = remainder[j] / divisor[divisorDegree];
           for (int i = divisorDegree; i >= 0; i--) {
               buffer[i + (bufferDegree - divisorDegree)] = divisor[i] * quotient[quotientDegree];
           }
