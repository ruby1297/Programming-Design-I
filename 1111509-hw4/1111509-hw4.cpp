    ...
    int remainderCoef[], int remainderExpon[], int& remainderSize)
{
    ...
    int bufferExpon[arraySize], bufferCoef[arraySize], bufferSize = divisorSize;
    quotientSize = dividendExpon[dividendSize - 1] - divisorExpon[divisorSize - 1] + 1;
    for (int i = quotientSize - 1; i >= 0; i--)
    {
        int h = remainderSize - 1, k = divisorSize - 1;
        quotientExpon[i] = remainderExpon[h] - divisorExpon[k];
        quotientCoef[i] = remainderCoef[h] / divisorCoef[k];
        h--;
        k--;
        int monomialExpon[1]{}, monomialCoef[1]{}, monomialSize = 1;
